...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen flex flex-col">
    {pathname !== "/intake" && pathname !== "/login" && <header data-screen className="care-nav">
      ...
      <div className="care-nav-actions"><ThemeToggle />{pathname === "/itinerary" && hasPlan ? <button type="button" className="care-nav-cta" onClick={() => window.dispatchEvent(new Event("carecation:open-print-preview"))}>Print</button> : <button type="button" className="care-nav-cta" onClick={action}>{actionLabel}<Plane size={15} className="rotate-45" aria-hidden="true" /></button>}</div>
    </header>}
